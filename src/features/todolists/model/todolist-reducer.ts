...
import { Todolist } from "@/features/todolists/api/todolists-api.types.ts"

export const setTodolistsAC = createAction<{ todolists: Todolist[] }>("todolists/setTodo")
...
export const todolistReducer = createReducer(initialState, (builder) => {
  builder
    ...
    .addCase(setTodolistsAC, (state, action) => {
      action.payload.todolists.forEach((todo) => state.push({ ...todo, filter: "all" }))
    })
})
